...
void setup(){Serial.begin(115200);Wire.begin(PagerConfig::OLED_SDA,PagerConfig::OLED_SCL);delay(100);if(!display.begin(PagerConfig::OLED_ADDRESS,true)){Serial.println("ERR OLED init failed");while(true)delay(1000);}encoder.begin();seed();view.begin(messages);view.render(messages);Serial.println("OK PAGER/0.1");}
