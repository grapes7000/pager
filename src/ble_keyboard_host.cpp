...
#ifdef PAGER_KEYBOARD_ADDRESS_PREFIX
constexpr char kKeyboardAddressPrefix[]=PAGER_KEYBOARD_ADDRESS_PREFIX;
#endif
const NimBLEUUID kHidService((uint16_t)0x1812);

bool addressStartsWith(const std::string& seen, const char* configured) {
 const size_t n=strlen(configured);
...
bool addressMatches(const std::string& seen, const char* configured) {
 return seen.size()==strlen(configured)&&addressStartsWith(seen,configured);
}
bool configuredAddressMatches(const std::string& seen) {
#ifdef PAGER_KEYBOARD_ADDRESS_PREFIX
 return addressStartsWith(seen,kKeyboardAddressPrefix);
#else
 return addressMatches(seen,kKeyboardAddress);
#endif
}
class ScanCallbacks:public NimBLEScanCallbacks{
 ...
 void onResult(const NimBLEAdvertisedDevice* d) override{
  ...
  bool addr=configuredAddressMatches(d->getAddress().toString());
...
void BleKeyboardHost::begin(){
 instance_=this;
#ifdef PAGER_KEYBOARD_ADDRESS_PREFIX
 Serial.printf("[BLE] starting BLE HID keyboard client; target-prefix=%s*\n",kKeyboardAddressPrefix);
#else
 Serial.printf("[BLE] starting BLE HID keyboard client; target=%s\n",kKeyboardAddress);
#endif
...
void BleKeyboardHost::startScan(){
 ...
 #ifdef PAGER_KEYBOARD_ADDRESS_PREFIX
 Serial.printf("[BLE] scanning for configured keyboard prefix %s*...\n",kKeyboardAddressPrefix);
#else
 Serial.printf("[BLE] scanning for configured keyboard %s...\n",kKeyboardAddress);
#endif
 scanning_=true;shouldConnect_=false;
